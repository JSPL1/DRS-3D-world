...
import { getPool, one } from '@/lib/db';
...
export async function PUT(req: Request) {
  ...
  const product = await one<{ slug: string; name: string }>(
    ...
    for (const row of rows) {
      await conn.query(
        `INSERT INTO product_colors (product_id, color_id, image_url, price_delta, is_default, sort_order)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [productId, row.colorId, row.imageUrl?.trim() || null, row.priceDelta, row.isDefault ? 1 : 0, row.sortOrder],
      );
    }
    await conn.commit();
  } catch (error) {
