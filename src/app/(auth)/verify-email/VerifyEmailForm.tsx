...
export function VerifyEmailForm({
  ...
  email: string;
  nextPath?: string;
  /** Set when we sent a fresh code on the way in, e.g. from a blocked sign-in. */
  ...
  const [notice, setNotice] = useState<string | null>(
    alreadyResent ? 'A fresh code has been sent to your email.' : null,
  );
  ...
  async function submit(code: string) {
    ...
    try {
      ...
      // Same-site paths only, so a crafted link can't bounce a freshly
      // authenticated customer off to another domain.
      const safeNext =
        nextPath && nextPath.startsWith('/') && !nextPath.startsWith('//') ? nextPath : null;

      router.push(safeNext ?? data.redirectTo ?? '/account');
      router.refresh();
    } catch {
  ...
  async function resend() {
    ...
    try {
      ...
      setNotice(
        data.devCode
          ? `Development mode — new code is ${data.devCode}`
          : (data.message ?? 'A new code is on its way.'),
      );
    } catch {
      setError('Network problem. Please try again.');
    }
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold tracking-tight">Confirm your email</h1>
        <p className="mt-2 text-[14px] leading-relaxed text-ink-400">
          We sent a six-digit code to <span className="text-white">{email}</span>. Enter it and your
          account is ready.
        </p>
      </div>
